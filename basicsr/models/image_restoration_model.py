...
class ImageRestorationModel(BaseModel):
    ...
    def grids(self):
        ...
        import math
        step_j = 384 #crop_size if num_col == 1 else math.ceil((w - crop_size) / (num_col - 1) - 1e-8)
        step_i = 384 #crop_size if num_row == 1 else math.ceil((h - crop_size) / (num_row - 1) - 1e-8)
    ...
    def single_image_inference(self, img, save_path):
       self.feed_data(img) # data={'lq': img.unsqueeze(dim=0)})
       ...
       imwrite(sr_img, save_path, auto_mkdir=False)
